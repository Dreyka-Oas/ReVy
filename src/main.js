...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  const showBlacklist = () => {
    const raw = JSON.parse(localStorage.getItem('lethalbreed_raw_entries') || '[]');
  ...
  const setActive = async (path, refreshList = false) => {
    toggleVersionsPanel(false);
    toggleSettingsPanel(false);
    state = await updateActivePath(state, path);
    if (refreshList) {
      entries = await performSync(state);
      await render();
    } else {
      document.querySelectorAll('.settings-item[data-path]').forEach(item => item.classList.toggle('active', item.dataset.path === path));
    }
    await runScan(path);
    await refreshHeader(path);
  };
  const runScan = async (p, showBtnOnError = true) => {
    const el = document.getElementById("path-display");
    if (el) {
      el.textContent = p;
      el.classList.remove("hidden");
      let existingNotif = el.querySelector(".copy-notif");
      el.onclick = () => {
        ...
        setTimeout(() => { notif.remove(); existingNotif = null; }, 1500);
      };
    }
    showLoading();
    try {
      const s = await detectLoaders(p);
      currentTestLoaders = s.loaders ?? [];
      await showLoaders(s.loaders ?? [], s.debug ?? [], selectLoader);
      document.getElementById('action-buttons')?.classList.toggle('hidden', !(s.loaders && s.loaders.length > 0));
      if ((s.loaders?.length ?? 0) === 0 && showBtnOnError) updatePickerButton(true);
    } catch (e) {
      console.error('[Scan] Error:', e);
      if (showBtnOnError) updatePickerButton(true);
    }
  };
  const selectLoader = async loader => {
    try {
      let versions = getCachedVersions(loader.path);
      if (!versions) { const entries = await readDir(loader.path); versions = entries.filter(e => e.isDirectory).map(e => e.name).sort((a, b) => b.localeCompare(a, undefined, { numeric: true, sensitivity: 'base' })); saveCachedVersions(loader.path, versions); }
      renderVersions(versions, loader.name, loader.icon, loader.path, () => toggleVersionsPanel(false), async (version, loaderPath, playBtn) => {
        ...
        try { await launchMinecraftVersion(loaderPath, version, min, max); } catch (e) { console.error('[Launch] Error:', e); } finally { if (playBtn) setTimeout(() => { playBtn.disabled = false; playBtn.classList.remove('launching'); if (originalLabel) playBtn.innerHTML = originalLabel; }, 600); }
      });
  ...
  if (testBtn) testBtn.onclick = async () => { setTestGlobalLoaders(currentTestLoaders); renderJavaRamPanel(await getSystemRam(), (min, max) => saveJavaRamConfig(min, max), async selected => await runTestGlobal(selected, updateTestProgress), enabled => setTestLogEnabled(enabled)); toggleSettingsPanel(false); };

  const sdkmanInstalled = await checkSdkman(), javaBtn = document.getElementById('java-btn');
  const loadJavaPanel = async (forceRefresh = false) => {
    const { installed, available } = await loadJavaData(forceRefresh);
    renderJavaPanel(installed, available, () => toggleVersionsPanel(false), async identifier => await installJavaWithProgress(identifier, setJavaInstalling, () => loadJavaPanel(true), async () => { setJavaInstalling(identifier, -1); await loadJavaPanel(true); }), () => loadJavaPanel(true), async identifier => await uninstallJavaWithProgress(identifier, setJavaInstalling, () => loadJavaPanel(true), async () => { setJavaInstalling(identifier, -1); await loadJavaPanel(true); }));
  };
  ...
  await sync(); // Use cache if available at startup
  ...
  if (state.activePath) { 
    // Show something immediately if we have a path
    await refreshHeader(state.activePath);
    await setActive(state.activePath); 
  }
  else if (entries.length > 0) { 
    await setActive(entries[0].path); 
  }
  else { 
